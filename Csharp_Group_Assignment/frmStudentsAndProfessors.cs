    ...
    public partial class frmStudentsAndProfessors : Form
    {
        ...
        public frmStudentsAndProfessors()
        {
            ...
            picReport.Click += (sendeer, args) =>
            {
                ...
                switch (Current.Value)
                {
                    case LinkType.Students:
                        ...
                        foreach (dynamic person in info.Student.All)
                        {
                            ...
                            foreach (var personLink in person.Links)
                            {
                                ...
                                switch (personLink)
                                {
                                    case Course _:
                                        ...
                                        linkHTML[typeof(info.opportunity.Course)] += $"<tr><td>{((info.opportunity.Course) personLink).ID}</td>"
                                                                    + $"<td>{((info.opportunity.Course) personLink).Name}</td>"
                                                                    + $"<td>{((info.opportunity.Course) personLink).Capacity}</td>"
                                                                    + $"<td>{Math.Round(((info.opportunity.Course) personLink).Credits):F2}</td></tr>";
                                        break;
                    ...
                    case LinkType.Professors:
                        ...
                        foreach (dynamic person in info.Professor.All)
                        {
                            pdfHTML += $"<tr><td>{person.ID}</td>"
                                       + $"<td>{person.FullName}</td>"
                                       + $"<td>{person.BirthDate:YYYY-MM-DD}</td>"
            ...
            picLink.Click += (sendeer, args) =>
            {
                ...
                {
                    new StudentLink((info.Student) edgStudents.Selected).ShowDialog();
                }
                else if (_current.Value == LinkType.Professors)
                {
                    new ProfessorLink((info.Professor) edgStudents.Selected).ShowDialog();
                }
            };
            ...
            picAdd.Click += (sender, args) =>
            {
                switch (_current)
                {
                    ...
                    case LinkType.Professors:
                        ...
                        var professor = new info.Professor();
